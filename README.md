...
ウィジェットからワンタップで起動できる iOS 専用の相対カウントダウンタイマー（英語名 **TimerTiles**）。AlarmKit で消音・集中モード中でも確実に鳴り、ホーム/ロック画面のウィジェットから起動。プリセットは複数のボード（欄）に振り分けでき、ウィジェットごとに表示ボードを選べる。買い切り Pro。
